...
public:
    /**
     * iteratively get the left boudary and right bounday, recursively get all leaf nodes.
     */
...
int main() {
    ...
    vector<struct d> p {
        {{1,INT_MAX,2,3,4}},
        {{1,2,3,4,5,6,INT_MAX,INT_MAX,INT_MAX,7,8,9,10}},
    };
    ...
    for (auto& dp : p) {
        ...
        auto res = boundaryOfBinaryTree::Solution().boundaryOfBinaryTree(toTree(dp.nodeVals));

...
public:
    /**
     * use a monotinic queue to record number in increasing order.
     * when a number force some elements out of the queue, check whether the
     * smallest number is smaller than current element.
     * This can't solve [3,5,0,3,4]
     */
    bool find132pattern_wrong(vector<int>& nums) {
        int n = nums.size();
        deque<int> q;
        int mmin = INT_MAX;
        for (int i = 0; i < n; ++i) {
